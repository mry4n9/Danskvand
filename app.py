...
if st.button("✨ Generate Ad Content", type="primary", use_container_width=True):
    ...
    # ... (keep context extraction code as is) ...
    ...
    # Use a dictionary to track progress (mutable object)
    progress_state = {'completed': 0} 

    def update_progress(task_name):
        # No nonlocal needed - modify the dictionary's value
        progress_state['completed'] += 1 
        progress_value = 30 + int((progress_state['completed'] / total_tasks) * 65) # 30% for context, 65% for generation
        # Ensure progress doesn't exceed 95 before the final step
        progress_value = min(progress_value, 95) 
        progress_bar.progress(progress_value, text=f"Generating {task_name}...")

    # --- Start Generation ---
    try: # Add a try block for better error handling during generation
        ...
        progress_bar.progress(95, text="Formatting Excel report...") # Set to 95 explicitly before final step
